...
#define UART_TX_IN_PROGRESS(_udp_)                                  \
    osal_start_timeout(&((_udp_)->timeout));                        \
...
#define UART_WAIT_FOR_TX_COMPLETION(_udp_)                          \
    if (((_udp_)->tx_drv_mode == UART_TX_DRV_MODE_INT_SYNC) ||      \
        ...
        while ((_udp_)->tx_busy == UART_TX_BUSY) {                  \
            if (osal_check_timeout((_udp_)->timeout) == true) {     \
                (_udp_)->status = UART_STATUS_TIMEOUT;              \
                break;                                              \
            }                                                       \
        }                                                           \
    }
...
static void uart_reset_paramters(uart_driver_t *udp) {
    ...
    udp->dma_conf.dma_stream_tx_id    = DMA1_STREAM0_ID;
    udp->dma_conf.dma_stream_rx_id    = DMA1_STREAM1_ID;
    udp->dma_conf.dma_stream_bus_prio = DMA_PRIORITY_LOW;
    udp->dma_conf.dma_stream_irq_prio = IRQ_MIN_PRIORITY;
    ...
    udp->timeout.start                = 0UL;
    udp->timeout.value                = 0UL;
    udp->status                       = UART_STATUS_OK;
...
/**
 * @brief   Reads frames from UART in interrupt mode.
 *
 * @param[out] udp      pointer to a @p uart_driver_t structure
 *
 * @notapi
...
/**
 * @brief   Reads frames from UART in buffered mode.
 *
 * @param[out] udp      pointer to a @p uart_driver_t structure
 *
 * @notapi
 */
static void uart_buffered_lld_read(uart_driver_t *udp) {

    uint32_t len = udp->rx_len;

    osal_start_timeout(&(udp->timeout));
    ...
    /* Wait till all required bytes are read from the Synchronous Buffered
       mode buffer.*/
    while (len != 0U) {
        /* Check the timeout if enabled.*/
        if (osal_check_timeout(udp->timeout) == true) {
            udp->status = UART_STATUS_TIMEOUT;
            break;
        }
        ...
        /* ... otherwise re-enable data receive interrupt.*/
            ...
            udp->uart->CR1 |= UART_CR1_RXNEIE_RXFNEIE;
            /* NOP needed to avoid incorrectly optimization in release mode.*/
            asm("NOP");
        }
    }
...
void uart_set_dma_conf(uart_driver_t *udp, uart_dma_conf_t *dma_conf) {

    udp->dma_conf.dma_stream_tx_id    = dma_conf->dma_stream_tx_id;
    udp->dma_conf.dma_stream_rx_id    = dma_conf->dma_stream_rx_id;
    udp->dma_conf.dma_stream_bus_prio = dma_conf->dma_stream_bus_prio;
    udp->dma_conf.dma_stream_irq_prio = dma_conf->dma_stream_irq_prio;
}
...
uint32_t uart_set_timeout(uart_driver_t *udp, uint32_t timeout) {

    uint32_t prev_timeout;

    prev_timeout = osal_tk2ms(udp->timeout.value);
    udp->timeout.value = osal_ms2tk(timeout);
...
size_t uart_read(uart_driver_t *udp, uint8_t *data, size_t len) {
    ...
    if (len > 0UL) {
        ...
        } else if (udp->rx_drv_mode == UART_RX_DRV_MODE_INT_BUFF_SYNC) {
            uart_buffered_lld_read(udp);
        } else if (udp->is_fifo_en == true) {
...
uart_status_t uart_get_status(uart_driver_t *udp) {

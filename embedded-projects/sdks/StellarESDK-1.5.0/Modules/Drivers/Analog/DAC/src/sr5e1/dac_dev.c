...
/**
 * @brief   Returns DAC DMA channel 1 trigger.
 *
 * @param[in] ddp       pointer to a @p dac_driver_t structure
 * @return              DMA channel 1 trigger.
 *
 * @notapi
 */
uint32_t dac_dev_get_dma_ch1_trigger(dac_driver_t *ddp) {

    uint32_t dma_ch1_trigger;

    if (ddp == &DRV_DAC1) {
        dma_ch1_trigger = DMAMUX1_DAC1_CH1;
    } else if (ddp == &DRV_DAC2) {
        dma_ch1_trigger = DMAMUX1_DAC2_CH1;
    } else if (ddp == &DRV_DAC3) {
        dma_ch1_trigger = DMAMUX1_DAC3_CH1;
    } else if (ddp == &DRV_DAC4) {
        dma_ch1_trigger = DMAMUX1_DAC4_CH1;
    } else {
        dma_ch1_trigger = DMAMUX1_BDAC1_CH1;
    }

    return dma_ch1_trigger;
}

/**
 * @brief   Returns DAC DMA channel 2 trigger.
 *
 * @param[in] ddp       pointer to a @p dac_driver_t structure
 * @return              DMA channel 1 trigger.
 *
 * @notapi
 */
uint32_t dac_dev_get_dma_ch2_trigger(dac_driver_t *ddp) {

    uint32_t dma_ch2_trigger;

    if (ddp == &DRV_DAC1) {
        dma_ch2_trigger = DMAMUX1_DAC1_CH2;
    } else if (ddp == &DRV_DAC2) {
        dma_ch2_trigger = DMAMUX1_DAC2_CH2;
    } else if (ddp == &DRV_DAC3) {
        dma_ch2_trigger = DMAMUX1_DAC3_CH2;
    } else if (ddp == &DRV_DAC4) {
        dma_ch2_trigger = DMAMUX1_DAC4_CH2;
    } else {
        dma_ch2_trigger = DMAMUX1_BDAC1_CH2;
    }

    return dma_ch2_trigger;
}

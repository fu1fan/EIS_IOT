...
static void sdadc_reset_paramters(sdadc_driver_t *sdp) {
    ...
    sdp->is_trg_en          = false;
    sdp->trg_sel            = SDADC_TSEL_ADC1_SW_TRGO;
    sdp->trg_edge           = SDADC_TEDGE_FALLING_EDGE;
    sdp->is_ts_en           = false;
    sdp->timestamp          = 0U;
 ...
 * @notapi
 */
static sdadc_fsize_t sdadc_lld_get_fsize(sdadc_driver_t *sdp) {

    uint8_t fsize;

    fsize = (uint8_t)((sdp->sdadc->SDADC_FCR >> SDADC_SDADC_FCR_FSIZE_Pos) & SDADC_SDADC_FCR_FSIZE_Msk);

    switch (fsize) {
        case 0U:
            return SDADC_FIFO_SIZE_1;
        case 1U:
            return SDADC_FIFO_SIZE_4;
        case 3U:
            return SDADC_FIFO_SIZE_8;
        default:
            return SDADC_FIFO_SIZE_16;
    }
}
...
bool sdadc_enable_timestamp(sdadc_driver_t *sdp) {
    ...
    prev_is_ts_en = sdp->is_ts_en;
    sdp->is_ts_en = true;

    return prev_is_ts_en;
}
...
void sdadc_start(sdadc_driver_t *sdp) {
    ...
    /* Get FIFO size.*/
    sdp->fsize = sdadc_lld_get_fsize(sdp);
    ...
    /* Configure trigger.*/
    sdp->sdadc->SDADC_MCR &= ~SDADC_SDADC_MCR_TRIGEN;
    sdp->sdadc->SDADC_MCR &= ~SDADC_SDADC_MCR_TRIGSEL;
    sdp->sdadc->SDADC_MCR &= ~SDADC_SDADC_MCR_TRIGEDSEL;
    if (sdp->is_trg_en == true) {
        sdp->sdadc->SDADC_MCR |= SDADC_MCR_TRIGSEL(sdp->trg_sel);
        sdp->sdadc->SDADC_MCR |= SDADC_MCR_TRIGEDSEL(sdp->trg_edge);
    }

    /* Configure caputure timestamp request.*/
    sdp->sdadc->SDADC_RSER &=~ SDADC_SDADC_RSER_CTRE;
    if (sdp->is_ts_en == true) {
        sdp->sdadc->SDADC_RSER |= SDADC_SDADC_RSER_CTRE;
    }
}
...
void sdadc_stop_conversion(sdadc_driver_t *sdp) {
    ...
    /* If enabled, disable trigger mode.*/
    if (sdp->is_trg_en == true) {
        sdp->sdadc->SDADC_MCR &= ~(SDADC_SDADC_MCR_TRIGEN);
    }
...
bool sdadc_read(sdadc_driver_t *sdp, float *value) {
    ...
    if ((sdp->sdadc->SDADC_SFR & SDADC_SDADC_SFR_DFEF) == 0U) {
        ...
        *value = (*value) / sdp->gain_calibration;
        if(sdp->is_ts_en == true){
            sdp->timestamp = (uint32_t)(sdp->sdadc->SDADC_CDR);
        }
        return true;
    }
...
void __sdadc_serve_interrupt(sdadc_driver_t *sdp) {
    ...
    uint32_t rser = sdp->sdadc->SDADC_RSER;

    /* Check FIFO full event.*/
    if (((rser & SDADC_SDADC_RSER_DFFDIRE) != 0U) &&
        ((sfr & SDADC_SDADC_SFR_DFFF) != 0U)) {
        /* callback*/
    ...
    if (((rser & SDADC_SDADC_RSER_DFORIE) != 0U) &&
        ((sfr & SDADC_SDADC_SFR_DFORF) != 0U)) {
        /* callback*/
    ...
    if (((rser & SDADC_SDADC_RSER_WTHDIRE) != 0U) &&
        (((sfr & SDADC_SDADC_SFR_WTHH) != 0U) ||

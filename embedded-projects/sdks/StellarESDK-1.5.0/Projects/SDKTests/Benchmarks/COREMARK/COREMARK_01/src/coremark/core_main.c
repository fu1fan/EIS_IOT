...
#if MAIN_HAS_NOARGC
MAIN_RETURN_TYPE bCoreMark(void) {
	int argc=0;
	char *argv[1];
#else
MAIN_RETURN_TYPE bCoreMark(int argc, char *argv[]) {
#endif
...
#error "Please define a way to initialize a memory block."
	...
	if (results[0].iterations==0) { 
		...
		while (secs_passed < (secs_ret)1) {
			...
			iterate(&results[0]);
			stop_time();
			secs_passed=time_in_secs(delta_time());
		}
	...
	if (time_in_secs(total_time) > 0)
		ee_printf("Iterations/Sec   : %f\n",(double)(default_num_contexts*results[0].iterations/time_in_secs(total_time)));
#else 
	...
	ee_printf("Parallel %s : %u\n",PARALLEL_METHOD,(unsigned int)default_num_contexts);
...
#if (MULTITHREAD>1)
			ee_printf(" / %u:%s\r",(unsigned int)default_num_contexts,PARALLEL_METHOD);
#endif
			ee_printf("\n");
		}
#endif

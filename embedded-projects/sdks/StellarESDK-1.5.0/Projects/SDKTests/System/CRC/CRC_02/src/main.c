...
/* CRC expected value.*/
#define CRC_EXPECTED_VALUE                  0xB553F395U

volatile uint8_t crc_completed = 0U;
...
static void crc_config(crc_driver_t *cdp) {
    ...
    (void)crc_set_rev_in(cdp, CRC_REV_IN_NONE);
    (void)crc_set_rev_out(cdp, CRC_REV_OUT_NONE);

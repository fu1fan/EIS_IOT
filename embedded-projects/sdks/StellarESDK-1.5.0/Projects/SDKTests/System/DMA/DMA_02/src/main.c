...
int main(void)
{
    ...
    /* Invalidate value of variable destination in DCache. This means the value
       of variable destination will be update from RAM.
       RAM:    source[i] = i, destination[i] = i
       DCache: source[i] = i, destination[i] = i */
    SCB_InvalidateDCache_by_Addr(destination, DMA_BUFFER_SIZE);

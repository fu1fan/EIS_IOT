...
uint8_t spi_master_rx_buffer[SPI_BUFFER_SIZE];
uint8_t spi_slave_tx_buffer[SPI_BUFFER_SIZE];

/* This callback is invoked when SPI Master data receiving is completed.*/
static void spi_master_data_cb(spi_driver_t *spd) {

    (void)spd;

    /* Set spi_master_completed to indicate SPI Master data receiving is
       completed.*/
    spi_master_completed = 1;
}

/* This callback is invoked when SPI Slave data trasferring is completed.*/
static void spi_slave_data_cb(spi_driver_t *spd) {

    (void)spd;

    /* Set spi_slave_completed to indicate SPI Slave data trasferring is
       completed.*/
...
int main(void) {
    ...
    (void)spi_set_prio(&MASTER_SPID, IRQ_PRIORITY_9);
    (void)spi_set_drv_mode(&MASTER_SPID, SPI_DRV_MODE_INT_ASYNC);
    (void)spi_set_mode(&MASTER_SPID, SPI_MODE_MASTER);
    (void)spi_set_cmode(&MASTER_SPID, SPI_CMODE_HALF_DUPLEX);
    (void)spi_set_baud(&MASTER_SPID, SPI_BAUDRATE_DIV128);
    ...
    (void)spi_set_prio(&SLAVE_SPID, IRQ_PRIORITY_10);
    (void)spi_set_drv_mode(&SLAVE_SPID, SPI_DRV_MODE_INT_ASYNC);
    (void)spi_set_mode(&SLAVE_SPID, SPI_MODE_SLAVE);
    (void)spi_set_cmode(&SLAVE_SPID, SPI_CMODE_HALF_DUPLEX);
    (void)spi_set_baud(&SLAVE_SPID, SPI_BAUDRATE_DIV64);
    ...
    for ( ; ; ) {
        ...
        /* Initialize SPI Master RX buffer and SPI Slave TX buffer.*/
        for (i = 0; i < SPI_BUFFER_SIZE; i++ ) {
            spi_master_rx_buffer[i] = 0x0;
            spi_slave_tx_buffer[i] = i + j;
        }

        j = j + SPI_BUFFER_SIZE;

        /* Start data trasferring via SPI Slave.*/
        (void)spi_send(&SLAVE_SPID, (uint8_t *)spi_slave_tx_buffer, SPI_BUFFER_SIZE);

        /* Start data receiving via SPI Master.*/
        (void)spi_receive(&MASTER_SPID, (uint8_t *)spi_master_rx_buffer, SPI_BUFFER_SIZE);

        /* Wait till SPI Master data transmitting is completed.*/
        do {
            ;
        } while(spi_master_completed == 0);
    
        spi_master_completed = 0;

        /* Check data received by SPI Master.*/
        for (i = 0; i < SPI_BUFFER_SIZE; i++) {
            if (spi_master_rx_buffer[i] != spi_slave_tx_buffer[i]) {
                /* If data received by SPI Master is different from data
                   trasmitted by SPI Slave, then the application will stuck in

 ...
 */
typedef enum{
    ADC_AW_1 = 0,                      /**< ADC analog watchdog 1 */
    ADC_AW_2 = 1,                      /**< ADC analog watchdog 2 */
    ADC_AW_3 = 2,                      /**< ADC analog watchdog 3 */
} adc_aw_t;

/**
 * @enum adc_dual_mode_t
 * ADC dual mode configurations.
 */
typedef enum{
    ADC_INDEPENDENT_MODE     = 0x0,    /**< ADC independent mode        */
    ADC_REG_INJ_MODE         = 0x1,    /**< ADC regular + injected mode */
    ADC_REG_ALTTRG_MODE      = 0x2,    /**< ADC regular + alternate trigger mode*/
    ADC_INTRLVD_INJ_MODE     = 0x3,    /**< ADC interleaved + injected mode*/
    ADC_INJ_INJ_MODE         = 0x5,    /**< ADC injected + injected mode*/
    ADC_REG_REG_MODE         = 0x6,    /**< ADC regular + regular mode*/
    ADC_INTRLVD_INTRLVD_MODE = 0x7,    /**< ADC interleaved + interleaved mode*/
    ADC_ALTTRG_ALTTRG_MODE   = 0x9,    /**< ADC alternate trigger + alternate trigger mode*/
} adc_dual_mode_t;
...
typedef enum {
    ...
    ADC_OP_DUAL_MODE_CONV,             /**< Dual conversion event       */
...
/**
 * @brief   Configures ADC dual mode.
 * @note    Only available for ADC1-2 and ADC3-4.
 *          This configuration becomes effective after adc_start.
 *
 * @param[out] adp      pointer to a @p adc_driver_t structure
 * @param[in] mode      dual mode configuration. It can be one of the following
 *                      values:
 *            @arg @ref ADC_INDEPENDENT_MODE
 *            @arg @ref ADC_REG_INJ_MODE
 *            @arg @ref ADC_REG_ALTTRG_MODE
 *            @arg @ref ADC_INTRLVD_INJ_MODE
 *            @arg @ref ADC_INJ_INJ_MODE
 *            @arg @ref ADC_REG_REG_MODE
 *            @arg @ref ADC_INTRLVD_INTRLVD_MODE
 *            @arg @ref ADC_ALTTRG_ALTTRG_MODE
 * @return              previous dual mode configuration. It can be on of the
 *                      following values:
 *            @arg @ref ADC_INDEPENDENT_MODE
 *            @arg @ref ADC_REG_INJ_MODE
 *            @arg @ref ADC_REG_ALTTRG_MODE
...
/**
 * @brief   Configures dual mode ADC DMA configuration.
 * @note    This feature is available only for ADC_REG_REG_MODE dual mode.
 *          This configuration becomes effective after adc_start.
 *
 * @param[out] adp      pointer to a @p adc_driver_t structure
 * @param[in] dma_conf  pointer to DMA configuration to be configured
 * @param[in] buffer    pointer to DMA buffer
 *
 * @api
 */
void adc_set_dual_mode_dma_conf(adc_driver_t *adp, adc_dma_conf_t *dma_conf, uint32_t *buffer);
...
/**
 * @brief   Configures ADC analog watchdog thresholds and filter.
 * @note    This configuration becomes effective after adc_start and must be
 *          invoked after adc_set_data_res.
 *
 * @param[out] adp      pointer to a @p adc_driver_t structure
 * @param[in] watchdog  watchdog to be configured.
 *                      It can be one of the following values:
 *            @arg @ref ADC_AW_1
 *            @arg @ref ADC_AW_2
 *            @arg @ref ADC_AW_3
 * @param[in] aw_ltr    analog watchdog lower threshold to be configured.
 *                      Max value depends on the data resolution.
 * @param[in] aw_htr    analog watchdog higher threshold to be configured
 *                      Max value depends on the data resolution.
 * @param[in] filter    Number of samples violating the threshold required to
 *                      generate a flag/interrupt. Min = 1, Max = 8.
 *                @note The filter is available only when a signle ADC channel
 *                      is selected and only for ADC_AW_1. It will be ignored
 *                      for other instances.
 * @return              previous analog watchdog configuration.
 *
 * @api
 */
...
/**
 * @brief   Configures ADC analog watchdog monitored channels.
 * @note    This configuration becomes effective after adc_start.
 *
 * @param[out] adp      pointer to a @p adc_driver_t structure
 * @param[in] watchdog  watchdog to be configured.
 ...
 *            @arg @ref ADC_AW_2
 *            @arg @ref ADC_AW_3
 * @param[in] ch        channel to monitor
 * @return              previous monitored channels.
 *
 * @api
 */

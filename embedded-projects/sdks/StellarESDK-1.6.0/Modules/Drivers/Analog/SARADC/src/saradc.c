...
static void adc_reset_paramters(adc_driver_t *adp) {
    ...
    adp->is_reg_dma_en                = false;
    adp->dma_conf.dma_stream_id       = DMA1_STREAM0_ID;
    adp->dma_conf.dma_stream_bus_prio = DMA_PRIORITY_LOW;
    adp->dma_conf.dma_stream_irq_prio = IRQ_MIN_PRIORITY;
    adp->dma_descriptor               = NULL;
    /* Dual mode parameters.*/
    adp->is_dual_mode_en                 = false;
    adp->dual_mode                       = ADC_INDEPENDENT_MODE;
    adp->dm_dma_conf.dma_stream_id       = DMA1_STREAM0_ID;
    adp->dm_dma_conf.dma_stream_bus_prio = DMA_PRIORITY_LOW;
    adp->dm_dma_conf.dma_stream_irq_prio = IRQ_MIN_PRIORITY;
    adp->dm_dma_descriptor               = NULL;
    adp->dm_buffer                       = NULL;
 ...
 * @notapi
 */
static void adc_dm_dma_callback(void *arg, uint32_t sts) {
    ...
    if ( adp->op_cb[ADC_OP_DUAL_MODE_CONV] != NULL) {
        adp->op_cb[ADC_OP_DUAL_MODE_CONV](adp);
    }
}
...
void adc_set_dma_conf(adc_driver_t *adp, adc_dma_conf_t *dma_conf) {

    adp->is_reg_dma_en = true;

    adp->dma_conf.dma_stream_id       = dma_conf->dma_stream_id;
    adp->dma_conf.dma_stream_bus_prio = dma_conf->dma_stream_bus_prio;
    adp->dma_conf.dma_stream_irq_prio = dma_conf->dma_stream_irq_prio;
}
...
adc_dual_mode_t adc_set_dual_mode(adc_driver_t *adp, adc_dual_mode_t mode) {

    adc_dual_mode_t prev_mode;

    prev_mode = adp->dual_mode;
    adp->dual_mode = mode;

    return prev_mode;
}

void adc_set_dual_mode_dma_conf(adc_driver_t *adp, adc_dma_conf_t *dma_conf,
                                uint32_t *buffer) {

    adp->is_dual_mode_en = true;
    adp->dm_buffer = buffer;

    adp->dm_dma_conf.dma_stream_id       = dma_conf->dma_stream_id;
    adp->dm_dma_conf.dma_stream_bus_prio = dma_conf->dma_stream_bus_prio;
    adp->dm_dma_conf.dma_stream_irq_prio = dma_conf->dma_stream_irq_prio;
}
...
uint32_t adc_set_dif_sel(adc_driver_t *adp, uint32_t dif_mask) {

    uint32_t prev_dif_mask;
...
uint32_t adc_set_aw_tr(adc_driver_t *adp, adc_aw_t watchdog, uint16_t aw_ltr,
                       uint16_t aw_htr, uint8_t filter) {

    uint32_t prev_aw_tr;
    uint32_t aw_tr_reg = 0U;
    uint8_t shift = 0U;

    adp->is_aw_en[watchdog] = true;

    prev_aw_tr = adp->aw_tr[watchdog];
    ...
    case ADC_DATA_RES_8:
        shift = 4U;
        break;
    case ADC_DATA_RES_10:
        shift = 2U;
        break;
    case ADC_DATA_RES_12:
        shift = 0U;
        break;
    default:
    ...
    if (watchdog == ADC_AW_1) {
        /* Shift the thresholds data, left alignment needed.*/
        aw_tr_reg = ((uint32_t)aw_ltr) << shift |
                  ((((uint32_t)aw_htr) << shift) << ADC_TR1_HT1_Pos);

        /* Apply the filter only if the watchdog is WD1.*/
        aw_tr_reg |= ((uint32_t)(--filter) << ADC_TR1_AWDFILT_Pos);
    } else {
        if (shift == 6U) {
            /* Shift the thresholds data, left alignment needed.*/
            aw_tr_reg = ((uint32_t)aw_ltr) << 2U |
                      ((((uint32_t)aw_htr) << 2U) << ADC_TR1_HT1_Pos);
        } else {
            aw_tr_reg = ((uint32_t)aw_ltr) |
                       (((uint32_t)aw_htr) << ADC_TR1_HT1_Pos);
        }
    }

    /* Compose the watchdog threshold register.*/
    adp->aw_tr[watchdog] = aw_tr_reg;

    return prev_aw_tr;
}

uint32_t adc_set_aw_ch(adc_driver_t *adp, adc_aw_t watchdog, uint8_t ch) {

    uint32_t prev_ch;

    prev_ch = adp->aw_ch[watchdog];
    if(watchdog == ADC_AW_1){
        adp->aw_ch[watchdog] = ch;
    } else {
        adp->aw_ch[watchdog] |= (uint32_t)1U << ch;
    }
...
void adc_start(adc_driver_t *adp) {
    ...
    /* Get common dual mode configuration.*/
    dual_mode = REG_READ_FIELD(adp->adc_common->CCR, ADC_CCR_DUAL);

    /* Get common DMA configuration.*/
    dmacfg = REG_READ_FIELD(adp->adc_common->CCR, ADC_CCR_DMACFG);
    mdma = REG_READ_FIELD(adp->adc_common->CCR, ADC_CCR_MDMA);

    /* Configure clock mode, prescaler and dual mode.*/
    adp->adc_common->CCR = ((uint32_t)adp->clk_mode)  << ADC_CCR_CKMODE_Pos |
                           ((uint32_t)adp->presc)     << ADC_CCR_PRESC_Pos  |
                           ((uint32_t)adp->dual_mode) << ADC_CCR_DUAL_Pos   |
                                      dual_mode       << ADC_CCR_DUAL_Pos   |
                                      dmacfg          << ADC_CCR_DMACFG_Pos |
                                      mdma            << ADC_CCR_MDMA_Pos;
    ...
    if (adp->n_reg_channels > 0U) {
        ...
        /* If enabled, configure dual mode DMA (for masters only).*/
        if((adc_dev_is_master(adp) == true) && (adp->is_dual_mode_en == true)) {
            dma_mode = DMA_CCR_PL_VALUE(adp->dm_dma_conf.dma_stream_bus_prio) |
                       DMA_CCR_PSIZE_WORD  | DMA_CCR_MSIZE_WORD               |
            ...
            /* Configure DMA circular mode for dual mode.*/
            adp->adc_common->CCR |= ADC_CCR_DMACFG;

            if(adp->data_res == ADC_DATA_RES_12 ||
               adp->data_res == ADC_DATA_RES_10) {
                adp->adc_common->CCR |= ADC_CCR_MDMA_1;
            } else {
                adp->adc_common->CCR |= ADC_CCR_MDMA;
            }

            /* Allocates DMA stream.*/
            /*lint -e9087 */
            adp->dm_dma_descriptor = dma_stream_take(adp->dm_dma_conf.dma_stream_id,
                                                     adp->dm_dma_conf.dma_stream_irq_prio,
                                                     adc_dm_dma_callback,
                                                     (void *)adp);
            /*lint +e9087 */

            /* Configure DMA stream peripheral addresses.*/
            dma_stream_set_peripheral(adp->dm_dma_descriptor,
                                     (uint32_t)(&adp->adc_common->CDR));

            /* Configure DMA ADC trigger.*/
            dma_trigger = adc_dev_get_dma_trigger(adp);
            dma_stream_set_trigger(adp->dm_dma_descriptor, dma_trigger);

            dma_stream_set_memory(adp->dm_dma_descriptor,
                                 (uint32_t)(adp->dm_buffer));
            dma_stream_set_count(adp->dm_dma_descriptor, adp->n_reg_channels);
            dma_stream_set_transfer_mode(adp->dm_dma_descriptor, dma_mode);
            dma_stream_enable(adp->dm_dma_descriptor);
        }

        if(adp->is_reg_dma_en == true){
        ...
        if (adp->dm_dma_descriptor != NULL) {
            /* Freeing and stopping dual mode DMA.*/
            dma_stream_free(adp->dm_dma_descriptor);
            dma_stream_disable(adp->dm_dma_descriptor);
        }
    ...
    /* Configure ADC watchdog 1.*/
    if(adp->is_aw_en[ADC_AW_1] == true) {
        /* Set thresholds and filter.*/
        adp->adc->TR1 = adp->aw_tr[ADC_AW_1];

        /* Set channels to monitor.*/
        if(adp->aw_ch[ADC_AW_1] == ADC_CH_ALL) {
            /* Monitor all channels.*/
            REG_SET_FIELD(adp->adc->CFGR, ADC_CFGR_AWD1SGL, 0U);
        } else {
            /* Monitor a single channel.*/
            REG_SET_FIELD(adp->adc->CFGR, ADC_CFGR_AWD1CH, adp->aw_ch[ADC_AW_1]);
            REG_SET_FIELD(adp->adc->CFGR, ADC_CFGR_AWD1SGL, 1U);
        }
    ...
    /* Configure ADC watchdog 2.*/
    if(adp->is_aw_en[ADC_AW_2] == true) {
        /* Set thresholds and filter.*/
        adp->adc->TR2 = adp->aw_tr[ADC_AW_2];

        /* Set channels to monitor.*/
        REG_SET(adp->adc->AWD2CR, adp->aw_ch[ADC_AW_2]);

        /* Enable analog watchdog interrupts.*/
        ier |= ADC_IER_AWD2IE;
    }

    /* Configure ADC watchdog 3.*/
    if(adp->is_aw_en[ADC_AW_3] == true) {
        /* Set thresholds and filter.*/
        adp->adc->TR3 = adp->aw_tr[ADC_AW_3];

        /* Set channels to monitor.*/
        REG_SET(adp->adc->AWD3CR, adp->aw_ch[ADC_AW_3]);
    ...
    /* If at least one channel has a sampling time set to 2.5 clcock cycle,
       additional clock cycle is required.*/
    for (i = 0U; i < (uint8_t)ADC_CH_NUM; i++) {
        if (adp->smp_time[i] == ADC_SMP_2P5) {
            smpr1 |= ADC_SMPR1_SMPPLUS;
            break;
        }
    }
    adp->adc->SMPR1 = smpr1;
...
void adc_start_regular(adc_driver_t *adp) {
    ...
    if(adp->is_aw_en[ADC_AW_1] == true){
        REG_SET_FIELD(adp->adc->CFGR, ADC_CFGR_AWD1EN, 1U);
    }
...
void adc_start_injected(adc_driver_t *adp) {
    ...
    if(adp->is_aw_en[ADC_AW_1] == true){
        REG_SET_FIELD(adp->adc->CFGR, ADC_CFGR_JAWD1EN, 1U);
    }
...
void adc_stop(adc_driver_t *adp) {
    ...
    /* If enabled, stop dual mode conversions.*/
    if (adp->is_dual_mode_en == true) {
...
void __adc_serve_interrupt(adc_driver_t *adp) {
    ...
    /* AWD1 event.*/
    if ((isr & ADC_ISR_AWD1) == ADC_ISR_AWD1) {
        /* Clear interrupt flag.*/
        adp->adc->ISR |= ADC_ISR_AWD1;

        adc_normal_error_event(adp, ADC_ERR_AWD1);
    }

    /* AWD2 event.*/
    if ((isr & ADC_ISR_AWD2) == ADC_ISR_AWD2) {
        /* Clear interrupt flag.*/
        adp->adc->ISR |= ADC_ISR_AWD2;

        adc_normal_error_event(adp, ADC_ERR_AWD2);
    }

    /* AWD3 event.*/
    if ((isr & ADC_ISR_AWD3) == ADC_ISR_AWD3) {
        /* Clear interrupt flag.*/
        adp->adc->ISR |= ADC_ISR_AWD3;

        adc_normal_error_event(adp, ADC_ERR_AWD3);
    }

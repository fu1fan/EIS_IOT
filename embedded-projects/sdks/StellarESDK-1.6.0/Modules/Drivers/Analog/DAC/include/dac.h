...
typedef enum {
    ...
    DAC_DVR_MODE_DOUBLE_DATA,     /**< DAC double data mode */
...
typedef enum {
    ...
    DAC_DMA_ON,                   /**< DMA normal mode enabled      */
    DAC_DMA_DD_ON                 /**< DMA double data mode enabled */
} dac_dma_mode_t;

/**
 * @brief   Type of DAC DMA configuration.
 */
typedef struct {
    /**
     * @brief   DAC DMA stream to be used for operations.
     */
    uint32_t                 dma_stream_id;
    /**
     * @brief   DAC DMA stream bus priority (3..0).
     */
    uint32_t                 dma_stream_bus_prio;
    /**
     * @brief   DAC DMA stream IRQ priority (3..15).
     */
...
/**
 * @brief   Sets DAC digital values.
 * @note    It must be invoked after dac_start, otherwise it doesn't not take
 *          effect.
 * @note    In single mode, only data1 take effect according to the channel
 *          number. In double data mode, both data1 and data2 take effect
 *          according to the channel number. In dual mode, both data1 and data2
 *          take effect ignoring channel number.
 *
 * @param[out] ddp      pointer to a @p dac_driver_t structure
 * @param[in] channel   DAC dedicated channel to be set. It can be one of the
 *                      following values:
 *            @arg @ref DAC_CHANNEL_1
 *            @arg @ref DAC_CHANNEL_2
 * @param[in]  data1    DAC 1st digital value
 * @param[in]  data2    DAC 2nd digital value
 *
 * @api
 */
...
/**
 * @brief   Sets DAC digital values in DMA mode.
 * @note    It must be invoked after dac_start, otherwise it doesn't not take
 *          effect.
 * @note    In order to use this API, DMA must be previuosly configured and
 *          enabled on DAC channel.
 *
 * @param[out] ddp      pointer to a @p dac_driver_t structure
 * @param[in] channel   DAC dedicated channel to be set. It can be one of the
 *                      following values:
 *            @arg @ref DAC_CHANNEL_1
 *            @arg @ref DAC_CHANNEL_2
 * @param[in]  data     pointer to the buffer containing values to be set
 *                      through DMA
 * @param[in]  len      number of elements in the buffer
 *
 * @api
 */
 ...
 * @note    It can be invoked after enabling the calibration and requires a
 *          dac_start afterwards.
 *
 * @param[out] ddp      pointer to a @p dac_driver_t structure
 ...
 *            @arg @ref DAC_CHANNEL_1
 *            @arg @ref DAC_CHANNEL_2
 * @return              DAC user trimming value
 *
 * @api
 */

...
void dma_stream_set_callback(const dma_descriptor_t *ddp, dma_callback_t cb,
                             void *arg) {

    uint32_t stream_msk = (1UL << ddp->stream);

    /* If stream is taken, re-configures DMA handler.*/
    if ((dma.taken & stream_msk) != 0U) {
        dma.streams[ddp->stream].cb  = cb;
        dma.streams[ddp->stream].arg = arg;
    }
}

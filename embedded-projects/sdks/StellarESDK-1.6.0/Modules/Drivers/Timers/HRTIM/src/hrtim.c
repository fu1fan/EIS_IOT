...
static void hrtim_reset_paramters(hrtim_driver_t *hdp) {
    ...
    for (adctrigger = 0U; adctrigger < (uint8_t)HRTIM_ADCTRIG_NUM; adctrigger++) {
       hdp->adc_trg[adctrigger].adc_trigger = HRTIM_NO_ADCTRIGGER;
    ...
    for (flt_index = 0U; flt_index < (uint8_t)HRTIM_FAULT_NUM; flt_index++) {
		...
		hdp->fault_blnk[flt_index].blanking_enable = false;
        hdp->fault_blnk[flt_index].blanking_source = HRTIM_FAULTBLANKINGMODE_RSTALIGNED;
    }

    hdp->fault_prescaler = HRTIM_FAULTPRESCALER_DIV1;

    hdp->sync_in  = HRTIM_SYNCIN_NONE;
    hdp->sync_out = HRTIM_SYNCOUT_DISABLED;
    hdp->sync_src = HRTIM_SYNCSRC_MASTER_START;
    hdp->is_master_sync_start = false;
    hdp->is_master_sync_reset = false;
    for (timer = 0U; timer < (uint8_t)HRTIM_TIMER_NUM; timer++) {
        hdp->is_timer_sync_start[timer] = false;
        hdp->is_timer_sync_reset[timer] = false;
    }
}
...
void hrtim_start(hrtim_driver_t *hdp) {
    ...
    for (i = 0U; i < HRTIM_TIMER_NUM; i++) {
        ...
        for (eev_idx = 1U; eev_idx <= HRTIM_EXEVENT_NUM; eev_idx++) {
        ...
        if (hdp->is_timer_sync_start[i] == true) {
            cr |= HRTIM_TIMxCR_SYNCSTRTx;
        }

        if (hdp->is_timer_sync_reset[i] == true) {
            cr |= HRTIM_TIMxCR_SYNCRSTx;
        }
    ...
    for (i = 0U; i < HRTIM_ADCTRIG_NUM; i++) {
        switch (hdp->adc_trg[i].adc_trigger) {
            case HRTIM_ADCTRIGGER_1:
                ...
                hrtim_cr1 &= ~(HRTIM_CR1_ADC1USRC);
                hrtim_cr1 |= ((hdp->adc_trg[i].update_src << HRTIM_CR1_ADC1USRC_Pos) & HRTIM_CR1_ADC1USRC);
            ...
            case HRTIM_ADCTRIGGER_2:
                ...
                hrtim_cr1 |= ((hdp->adc_trg[i].update_src << HRTIM_CR1_ADC2USRC_Pos) & HRTIM_CR1_ADC2USRC);
            ...
            case HRTIM_ADCTRIGGER_3:
                ...
                hrtim_cr1 |= ((hdp->adc_trg[i].update_src << HRTIM_CR1_ADC3USRC_Pos) & HRTIM_CR1_ADC3USRC);
            ...
            case HRTIM_ADCTRIGGER_4:
                ...
                hrtim_cr1 |= ((hdp->adc_trg[i].update_src << HRTIM_CR1_ADC4USRC_Pos) & HRTIM_CR1_ADC4USRC);
    ...
    for (i = 1U; i <= HRTIM_EXEVENT_NUM; i++) {
        switch (i) {
            case HRTIM_EVENT_1:
    ...
    for (idx = 0U; idx < HRTIM_FAULT_NUM; idx++) {
        ...
        switch (hdp->fault[idx].fault_idx) {
            case HRTIM_FAULT_1:
                ...
                if (hdp->fault_blnk[idx].blanking_enable == true) {
            ...
            case HRTIM_FAULT_5:
                ...
                hdp->hrtim->sCommonRegs.FLTINR4 &=~ (HRTIM_FLTINR4_FLT5RSTM | HRTIM_FLTINR4_FLT5CNT);
                hdp->hrtim->sCommonRegs.FLTINR4 |= (((uint32_t)hdp->fault_blnk[idx].threshold << HRTIM_FLTINR4_FLT5CNT_Pos) | 
                                                    ((uint32_t)hdp->fault_blnk[idx].reset_mode << HRTIM_FLTINR4_FLT5RSTM_Pos));
                hdp->hrtim->sCommonRegs.FLTINR4 &= ~(HRTIM_FLTINR4_FLT5BLKS | HRTIM_FLTINR4_FLT5BLKE);
                if (hdp->fault_blnk[idx].blanking_enable == true) {
                    hdp->hrtim->sCommonRegs.FLTINR4 |= (((uint32_t)hdp->fault_blnk[idx].blanking_source << HRTIM_FLTINR4_FLT5BLKS_Pos) | HRTIM_FLTINR4_FLT5BLKE);
                }
                if (hdp->fault[idx].enable == true) {
            ...
            case HRTIM_FAULT_6:
                ...
                hdp->hrtim->sCommonRegs.FLTINR4 &=~ (HRTIM_FLTINR4_FLT6RSTM | HRTIM_FLTINR4_FLT6CNT);
                hdp->hrtim->sCommonRegs.FLTINR4 |= (((uint32_t)hdp->fault_blnk[idx].threshold << HRTIM_FLTINR4_FLT6CNT_Pos) | 
                                                    ((uint32_t)hdp->fault_blnk[idx].reset_mode << HRTIM_FLTINR4_FLT6RSTM_Pos));
                hdp->hrtim->sCommonRegs.FLTINR4 &= ~(HRTIM_FLTINR4_FLT6BLKS | HRTIM_FLTINR4_FLT6BLKE);
                if (hdp->fault_blnk[idx].blanking_enable == true) {
                    hdp->hrtim->sCommonRegs.FLTINR4 |= (((uint32_t)hdp->fault_blnk[idx].blanking_source << HRTIM_FLTINR4_FLT6BLKS_Pos) | HRTIM_FLTINR4_FLT6BLKE);
                }
                if (hdp->fault[idx].enable == true) {
    ...
    if (hdp->is_master_sync_start == true) {
        mstr_cr |= HRTIM_MCR_SYNCSTRTM;
    }

    if (hdp->is_master_sync_reset == true) {
        mstr_cr |= HRTIM_MCR_SYNCRSTM;
    }
    ...
    hdp->hrtim->sMasterRegs.MCR    = mstr_cr | (uint32_t)(hdp->master_prescaler)                     | 
                                               ...
                                               (uint32_t)(hdp->master_preload.update_event)          |
                                               ((uint32_t)(hdp->sync_in) << HRTIM_MCR_SYNC_IN_Pos)   |
                                               ((uint32_t)(hdp->sync_out) << HRTIM_MCR_SYNC_OUT_Pos) | 
                                               ((uint32_t)(hdp->sync_src) << HRTIM_MCR_SYNC_SRC_Pos);
...
void hrtim_timer_enable_output(hrtim_driver_t *hdp,  uint16_t omask) {
    ...
    hdp->hrtim->sCommonRegs.OENR = (0xFFFUL & omask);
}

void hrtim_timer_disable_output(hrtim_driver_t *hdp, uint16_t omask) {

    hdp->hrtim->sCommonRegs.ODISR = (0xFFFUL & omask);
}

void hrtim_timer_enable(hrtim_driver_t *hdp, uint8_t tmask) {

    if (hdp->is_enable == true) {
        hdp->hrtim->sMasterRegs.MCR |= (((uint32_t)tmask) << HRTIM_MCR_MCEN_Pos);
    }
}
...
void hrtim_timer_enable_update(hrtim_driver_t *hdp, uint8_t tmask) {
    ...
    /* Update on-fly.*/
    if (hdp->is_enable == true)  {
        hdp->hrtim->sCommonRegs.CR1 &= ~(((uint32_t)tmask) << HRTIM_CR1_MUDIS_Pos);
    }
}
...
void hrtim_timer_set_adc_trigger(hrtim_driver_t *hdp,
    ...
    if (hdp->is_enable == true) {
        ...
        switch (adc_trigger) {
            case HRTIM_ADCTRIGGER_1:
                hrtim_cr1 &= ~(HRTIM_CR1_ADC1USRC_Msk);
...
void hrtim_set_ex_event(hrtim_driver_t *hdp,
    ...
    if (eev_idx != HRTIM_EVENT_NONE) {
        hdp->eev[eev_idx - 1U].source = ext_source;
        hdp->eev[eev_idx - 1U].polarity = ext_polarity;
        hdp->eev[eev_idx - 1U].sensitivity = ext_sensitivity;
        hdp->eev[eev_idx - 1U].filter = ext_filter;
        hdp->eev[eev_idx - 1U].fast_mode = ext_fast_mode;
    }
...
void hrtim_set_fault_counter_cfg(hrtim_driver_t *hdp,
    ...
    hdp->fault_blnk[fault_idx].blanking_enable = blanking_enable;	
...
hrtim_syncin_t hrtim_set_sync_in(hrtim_driver_t *hdp, hrtim_syncin_t sync_in) {
    ...
    hrtim_syncin_t prev_sync_in;

    prev_sync_in = hdp->sync_in;
    hdp->sync_in = sync_in;
...
hrtim_syncout_t hrtim_set_sync_out(hrtim_driver_t *hdp,
    ...
    hrtim_syncout_t prev_sync_out;

    prev_sync_out = hdp->sync_out;
    hdp->sync_out = sync_out;

    return prev_sync_out;
}

hrtim_syncsrc_t hrtim_set_sync_src(hrtim_driver_t *hdp,
                                   hrtim_syncsrc_t sync_src) {

    hrtim_syncsrc_t prev_sync_src;

    prev_sync_src = hdp->sync_src;
    hdp->sync_src = sync_src;

    return prev_sync_src;
}

bool hrtim_enable_sync_start(hrtim_driver_t *hdp, hrtim_timer_t timer,
                             bool enable) {

    bool prev_sync_start;

    if (timer == HRTIM_TIMER_MASTER) {
        prev_sync_start = hdp->is_master_sync_start;
        hdp->is_master_sync_start = enable;
    } else {
        prev_sync_start = hdp->is_timer_sync_start[timer];
        hdp->is_timer_sync_start[timer] = enable;
    }

    return prev_sync_start;
}

bool hrtim_enable_sync_reset(hrtim_driver_t *hdp, hrtim_timer_t timer,
                             bool enable) {

    bool prev_sync_reset;

    if (timer == HRTIM_TIMER_MASTER) {
        prev_sync_reset = hdp->is_master_sync_reset;
        hdp->is_master_sync_reset = enable;
    } else {
        prev_sync_reset = hdp->is_timer_sync_reset[timer];
        hdp->is_timer_sync_reset[timer] = enable;
    }

    return prev_sync_reset;
}

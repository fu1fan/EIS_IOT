 ...
 */
typedef enum {
    UART_STATUS_OK,          /**< UART status = no error      */
    UART_STATUS_TIMEOUT      /**< UART status = timeout error */
} uart_status_t;
...
/**
 * @brief   Configures UART timeout [ms].
 * @note    Timeout is taken into account only for synchronous modes. So, in
 *          reception it is taken into account only if the RX driver mode is
 *          one of the following values
 *          UART_RX_DRV_MODE_INT_SYNC
 *          UART_RX_DRV_MODE_DMA_SYNC
 *          UART_RX_DRV_MODE_INT_BUFF_SYNC
 *          while in transmission it is taken into account only it the TX
 *          driver mode is one of the following values
 *          UART_TX_DRV_MODE_INT_SYNC
 *          UART_TX_DRV_MODE_DMA_SYNC
 *
 * @param[out] udp      pointer to a @p uart_driver_t structure
...
/**
 * @brief   Returns the UART status.
 *
 * @param[in] udp       pointer to a @p uart_driver_t structure
 * @return              UART status. It can be one of the following values:
 *            @arg @ref UART_STATUS_OK
 *            @arg @ref UART_STATUS_TIMEOUT
 *
 * @api
 */

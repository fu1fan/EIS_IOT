...
void can_dev_clock_enable(can_driver_t *cdp) {
    ...
    /* In order to enable the clock of the FDCANs Message RAM, all FDCAN
       instances must be enabled.*/
    /* Update the mask of enabled CAN instances and enable the clock of the
       selected FDCAN instance.*/
    if (cdp == &DRV_CAN1) {
...
void can_dev_clock_disable(can_driver_t *cdp) {
    ...
    /* Update the mask of enabled CAN instances and disable the clock of the
       selected FDCAN instance.*/
    if (cdp == &DRV_CAN1) {
        can_enabled &= ~(1U << 0U);
        clock_disable_FDCAN1();
    ...
    /* Disable the clock of the FDCANs Message RAM only if no other FDCAN
       instance is enabled.*/
    if(can_enabled == 0U) {

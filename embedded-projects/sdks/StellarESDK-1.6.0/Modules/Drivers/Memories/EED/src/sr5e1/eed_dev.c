...
bool eed_dev_flashunlock(uint8_t block, uint8_t space) {
    ...
    if (flash_getLock(&FLASHD0, C55_BLOCK_HIGH, (unsigned long *)(&blkLockState)) != ((uint32_t)C55_OK)) {
        return false;
    }

    blkLockState &= ~(1UL << block);

    if (flash_setLock(&FLASHD0, C55_BLOCK_HIGH, (unsigned long)blkLockState) != ((uint32_t)C55_OK)) {
        return false;
    }
...
bool eed_dev_flashblankcheck(uint32_t address, uint32_t size) {
    ...
    /* Blank check.*/
    returnCode = flash_blankCheck(&FLASHD0, address, size, (unsigned long *)(&failAddress), (unsigned long *)(&failData));

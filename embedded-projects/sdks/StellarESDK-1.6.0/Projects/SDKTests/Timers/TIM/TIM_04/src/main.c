...
/* TIM1 break event callback */
void tim1_bk_callback(tim_driver_t *tdp)
{
    (void)(tdp);
    /* Blink USER_LED_A.*/
...
int main(void) {
    ...
    (void)tim_set_bk(&TIMD, TIM_BKP_LOW);
    (void)tim_set_cb(&TIMD, TIM_CB_OPS_CC1, tim1_ch1_callback);
    (void)tim_set_cb(&TIMD, TIM_CB_OPS_BK, tim1_bk_callback);

    /* Apply TIM configuration.*/
    tim_start(&TIMD);

    /* Enable TIM1 channel 1 and TIM1 break event callbacks.*/
    tim_enable_channel_callback(&TIMD, 1U);
    tim_enable_break_callback(&TIMD);

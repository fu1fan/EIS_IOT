...
volatile uint8_t reg_conv_completed = 0U;
float temperature;
...
static void adc_reg_data_cb(adc_driver_t *adp) {
    ...
    temperature = tsens_get_temperature((uint16_t)adc_reg_buffer[0], (uint16_t)adc_reg_buffer[1]);
}

int main(void) {

    adc_dma_conf_t adc_dma_conf;
    /* Channels 13 and 14 refer to CTAT and PTAT readings */
    uint8_t reg_seq[ADC_REG_NCHANNELS] = {ADC_CH_IN13, ADC_CH_IN14};
    ...
    /* Enable SAR_TESTVREF_SEL and SELFTEST_EN to power up temperature sensor.*/
    SYSCFG->ST_VREF_SEL2 |= 0x80000003;
    SYSCFG->ST_VREF_SEL2 |= 0x00008000;
    ...
    /* Configure ADC callback.*/
    (void)adc_set_op_cb(&ADCD, ADC_OP_REG_CONV, adc_reg_data_cb);
    /* Configure low sampling time for TSENS ADC channels */
    adc_set_smp_time(&ADCD, ADC_CH_IN13, ADC_SMP_6P5);
    adc_set_smp_time(&ADCD, ADC_CH_IN14, ADC_SMP_6P5);
    ...
    /* Initialize Temperature sensor module module.*/
    tsens_init();

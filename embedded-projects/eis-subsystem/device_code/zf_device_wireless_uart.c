...
uint32 wireless_uart_write_string (const char *str)
{
    ...
    while(0 != len)
    {
        if(!gpio_read_pin(WIRLESS_UART_RTS_PIN))                                // 如果RTS为低电平 则继续发送数据
        {
...
uint8_t wireless_uart_init (void)
{
    ...
    gpio_clear_pin(WIRLESS_UART_RST_PIN);
    osal_delay_millisec(50U);
    gpio_set_pin(WIRLESS_UART_RST_PIN);
    osal_delay_millisec(50U);
    ...
    wireless_uart_write_base((uint8 *)"AT+CANCEL", 9);

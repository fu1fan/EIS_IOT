...
#define system_delay_ms(ms) osal_delay_millisec(ms)
...
// 参数说明     command         命令
// 返回参数     void
// 使用示例     tft180_write_command(0x2a);
// 备注信息     内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
//static void tft180_write_command (uint8_t command)
//{
//	TFT180_DC(0);
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     tft180 向液晶屏写 8bit 数据
// 参数说明     dat             数据
// 返回参数     void
// 使用示例     tft180_write_8bit_data(0x0C);
// 备注信息     内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void tft180_write_8bit_data (uint8 dat)
{
...
// 备注信息     内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void tft180_write_8bit_data_array (uint8 *dat, uint32 len)
{
    spi_write_8bit_array(LCD_SPI_INDEX, dat, len);
}

//-------------------------------------------------------------------------------------------------------------------
// 函数简介     tft180 向液晶屏写 16bit 数据
// 参数说明     dat             数据
// 返回参数     void
// 使用示例     tft180_write_16bit_data(x1);
// 备注信息     内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void tft180_write_16bit_data (uint16 dat)
{
...
// 备注信息     内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void tft180_write_16bit_data_array (uint16 *dat, uint32 len)
{
...
void tft180_init (void)
{
	...
	tim_set_comparator  (&DRV_TIM4, 2U, 50U * TFT180_BLK_DUTY / 100);
	tim_enable          (&DRV_TIM4);

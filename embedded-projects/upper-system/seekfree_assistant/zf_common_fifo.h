...
// 数据写入操作 不能在重置和写入操作时进行 这是因为这两种操作都会修改 FIFO 尾部
// 顺序读取操作 不能在清空和重置操作时进行 这是因为这两种操作会修改 FIFO 头部
// 尾部读取操作 不能在清空和重置以及写入操作时进行 这是因为这三种操作可能会变更 FIFO 尾部
// 读取清空操作 不能在清空和重置以及读取操作时进行 这是因为前两种操作会修改 FIFO 头部 后一种会出现数据异常
// 这是为了防止中断嵌套导致数据混乱
typedef enum
{
    FIFO_IDLE       = 0x00,                                                     // 空闲状态
